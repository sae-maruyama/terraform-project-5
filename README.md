...
リソース構成の詳細や秘密情報（平文）が含まれるため、セキュリティの観点から除外します。本プロジェクトでは S3 リモートバックエンドを使用して共有

- **.terraform/**：Terraform 作業ディレクトリ<br>
terraform init 時にダウンロードされるプロバイダー等のバイナリです。環境依存であり、容量も大きいため除外
...
- **.zip**：デプロイ用パッケージ<br>
Lambda へアップロードするために生成したバイナリです。Git には「コード（ソース）」を保存すべきであり、成果物は管理不要なため除外します。
